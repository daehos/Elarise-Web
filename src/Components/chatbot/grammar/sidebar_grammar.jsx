...
const SidebarGrammar = () => {
  ...
  const handleRenameChat = async (chatRoomId, newName) => {
    try {
      ...
      if (response.data.status === "success") {
        setChats(chats.map(chat => chat.id === chatRoomId ? { ...chat, chatRoomName: newName } : chat));
      }
    } catch (error) {
          ...
          chats.map((chat, index) => (
            <>
              <div
                className={
                  isOpen
                    ? "flex flex-row justify-between items-center mr-5"
                    : ""
                }
                key={chat.id}
              >
                <div
                  ...
                  className={
                    isOpen
                      ? "flex flex-col w-full  bg-white font-semibold drop-shadow text-black py-2 px-8 my-2 border-[4px] rounded-3xl hover:bg-gray-200 cursor-pointer flex-grow"
                      : "flex items-center bg-white font-semibold drop-shadow text-black p-2 my-2 border-[4px] rounded-2xl hover:bg-gray-200 cursor-pointer flex-grow"
                  }
                  style={{
                    borderColor: index % 2 === 0 ? "#EFB4D4" : "#F8C807",
                  }}
                >
